...
#https://api.videoindexer.ai/{location}/Accounts/{accountId}/Videos/{videoId}/Index[?language][&reTranslate][&includeStreamingUrls][&includedInsights][&excludedInsights][&includeSummarizedInsights][&accessToken]
    ...
    def generate_video_summary(self, video_id:str, length:str, style:str, includedFrames:str) -> None:
        '''
        Used to create Video Summary (Preview version) 
        Initiate generation of new video summary based on below paraments
        length: values can be Short, Medium or Long
        style: values can be Neutral, Casual or Formal
        includedFramse: values can be None or Keyframes
        
        :param video_id: The video ID
        :param length: values can be Short, Medium or Long
        :param style: values can be Neutral, Casual or Formal
        :param includedFrames: values can be None or Keyframes

        '''
        self.get_account_async() # if account is not initialized, get it

        url = f'{self.consts.ApiEndpoint}/{self.account["location"]}/Accounts/{self.account["properties"]["accountId"]}/' + \
              f'Videos/{video_id}/Summaries/Textual?deploymentName={self.consts.DeploymentName}&length={length}&style={style}&includedFrames={includedFrames}'
        ...
        response.raise_for_status()
        print(f"Video Summary Generation for {video_id=} started...")


    def get_video_summmaries_list(self, video_id:str) -> None:
        '''
        Calls the List Video Summaries API
        (https://api-portal.videoindexer.ai/api-details#api=Operations&operation=List-Video-Summaries)
        It first generates a new access token for the video scope.
        Prints the VideoSummary URL, otherwise throws exception
        ...
        print(f'Getting the player summaries URL for video {video_id}')
        ...
        url = f'{self.consts.ApiEndpoint}/{self.account["location"]}/Accounts/{self.account["properties"]["accountId"]}/' + \
              f'Videos/{video_id}/Summaries/Textual?PageNumber=0&PageSize=20&'

        response = requests.get(url, params=params)

        response.raise_for_status()

        summary_lists = response.json()
        print(f'Here are video summary lists: \n{summary_lists}')
    ...
    def get_video_summary(self, video_id:str, summary_id:str) -> None:
        '''
        Calls the getVideoSummary API
        (https://api-portal.videoindexer.ai/api-details#api=Operations&operation=Get-Video-Player-summaries)
        It first generates a new access token for the video scope.
        Prints the VideoSummary URL, otherwise throws exception
        ...
        print(f'Here is video summary result: \n{summary_result}')
